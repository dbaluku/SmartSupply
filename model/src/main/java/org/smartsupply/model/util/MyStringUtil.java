...
public class MyStringUtil {
	...
	public static String getStandardisedStudentName(String fullname) {
		...
		if (fullname.contains(" ")) {
			...
			String otherNames = fullname.replaceFirst(firstName, "");
			fullname = firstName.toUpperCase() + otherNames;
		}
		return fullname;
	}
    ...
    public static String checkAndAddPercent(String str) {

        if (StringUtils.isNotBlank(str)) {
            if (!str.substring(0, 1).equalsIgnoreCase("%")
                    && !str.substring(str.length() - 1).equalsIgnoreCase("%"))
                str = MyStringUtil.percent(str);
        }
