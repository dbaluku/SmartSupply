...
import org.smartsupply.api.DefaultConcepts;
...
/**
 * handles the liquibase update of the concepts into the database
 *
 * @author ctumwebaze
 */
public class ConceptLiquibaseChange implements CustomTaskChange {
    ...
    public void execute(Database database) throws CustomChangeException, UnsupportedChangeException {
        ...
        private void insertRelationshipBetweenConceptAndConceptCategory(
                Database database, Field conceptField, ConceptAnnotation conceptAnnotation) throws SQLException,
            IllegalArgumentException, IllegalAccessException {
        String[] conceptCategories = conceptAnnotation.ConceptCategories();

        if (conceptCategories != null) {
            Field[] conceptCategoryFields = DefaultConceptCategories.class.getFields();
    ...
    private void insertConcept(ConceptAnnotation conceptAnnotation, Database database,
                               Field conceptField) throws SQLException, IllegalArgumentException, IllegalAccessException {
        PreparedStatement preConditionStatement = database.getConnection()
                .prepareStatement("SELECT 1 FROM concepts WHERE id=?");

        preConditionStatement.setString(1, conceptAnnotation.id());

        String sql = "INSERT INTO concepts (id, concept_description, concept_name, record_status) " + "VALUES (?, ?, ?, ?)";
        ...
        ResultSet preConditionResult = preConditionStatement.executeQuery();
        if (preConditionResult != null && preConditionResult.next()) {
            int value = preConditionResult.getInt(1);
            preConditionResult.close();

            if (value == 1) {
                log.debug("Skipping Field >> " + conceptField.getName() + " because it already exists in database");
            } else {
                statement.execute();
            }
        } else {
