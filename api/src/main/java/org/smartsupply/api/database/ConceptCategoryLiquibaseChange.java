...
/**
 * handles the liquibase update of the ConceptCategories into the database
 *
 * @author ctumwebaze
 */
public class ConceptCategoryLiquibaseChange implements CustomTaskChange {

    private Logger log = LoggerFactory.getLogger(ConceptCategoryLiquibaseChange.class);
    ...
    public void execute(Database database) throws CustomChangeException, UnsupportedChangeException {
        ...
        if (fields != null) {
            for (Field field : fields) {
                ...
                ConceptCategoryAnnotation annotation = field.getAnnotation(ConceptCategoryAnnotation.class);
                if (annotation != null) {
                    try {
                        PreparedStatement preConditionStatement = database.getConnection()
                                .prepareStatement("SELECT 1 FROM concept_categories WHERE id=?");
                        ...
                        if (preConditionResult != null && preConditionResult.next()) {
                            ...
                            if (value == 1) {
                                log.debug("Skipping Field >> " + field.getName() + " because it already exists in database");
                            } else {
                                statement.execute();
                            }
                        } else {
