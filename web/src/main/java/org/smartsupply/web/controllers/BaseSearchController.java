...
import static org.smartsupply.web.WebUtils.*;
import static org.springframework.web.bind.annotation.RequestMethod.POST;

public abstract class BaseSearchController<T extends BaseSearchService<ModeObject, SearchParams>,
    ...
    public void paginate(SearchParams params, ModelMap modelMap) throws Exception {
        ...
        int count = getService().numberInSearch(params);
    ...
    public ModelAndView add(ModelMap modelMap) {
        try {
            ...
            addContentHeader(modelMap, "Add " + singularName());
            addMenuActivators(modelMap);
    ...
    public ModelAndView edit(@PathVariable("id") String id, ModelMap modelMap) {
        try {
            ...
            checkAndThrowIdNotFoundException(singularName(), modeObject);

            modelMap.put(objectKey(), modeObject);
            prepareModel(modeObject, modelMap);
            addContentHeader(modelMap, "Edit " + singularName());
            addMenuActivators(modelMap);
            return new ModelAndView(formName(), modelMap);
        } catch (Exception e) {
            logExceptionAndAddErrorMessage(modelMap, e);
    ...
    public ModelAndView delete(@PathVariable("ids") String ids, ModelMap modelMap) {
        try {
            ...
            addSystemMessage(modelMap, singularOrPluralName() + " Successfully Deleted");
        } catch (Exception e) {
            logExceptionAndAddErrorMessage(modelMap, e);
        }
        return view(modelMap);
...
//                existing.setDateCreated(new Date());
            ...
            logExceptionAndAddErrorMessage(modelMap, e);
            addContentHeader(modelMap, "Retry Add/Edit " + singularName());
            try {
    ...
    protected void prepareSearchModel(SearchParams searchParams, ModelMap modelMap) {
        try {
            modelMap.put(listKey(), getService().searchWithParams(searchParams, searchParams.getPageNo()));
